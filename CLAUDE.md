...
**The identity registry is a vendored, locally-optimized copy at `.punt-labs/ethos/`** — plain committed files, per the org standard (same pattern as `lux` and `cryptd`). It carries only the 8-member `quarry` team (jfreeman, claude, rmh, gvr, kpz, djb, mdm, adb — the roster the pairing tables below actually use) plus those identities' personalities, writing styles, talents, and roles, and `teams/quarry.yaml` only. `team: quarry` in `.punt-labs/ethos.yaml` is what bounds SessionStart context injection to this roster; the pruned copy is what keeps the plugin clone lean. There is deliberately **no `.vendor.yaml`**: `ethos vendor` computes a membership-connected closure, and because these 8 belong to `engineering` it always plans the full engineering-connected roster (29 identities at the time of DES-054) — so a refresh is `ethos vendor claude jfreeman rmh gvr kpz djb mdm adb --apply` followed by pruning back to the quarry-team closure (delete non-roster identities and their unreferenced attributes, keep `teams/quarry.yaml` only, delete the tool's `.vendor.yaml`, re-run `ethos doctor`). Runtime state (`missions/`, `missions.jsonl`, `sessions/`, `.biff`) stays gitignored. Roster changes go to `punt-labs/team` first (`teams/quarry.yaml`; `engineering.yaml` must not claim `punt-labs/quarry`), then sync `~/.punt-labs/ethos/teams/` and re-vendor. **Do not add the `punt-labs/team` submodule here** — Claude Code clones plugin repos with `--recurse-submodules`, and `ethos enable` v4.15.0+ refuses submodule mounts outright (`ethos-e29s`).
