"""HTML format extraction: parse, strip boilerplate, convert to Markdown sections."""

from __future__ import annotations

import logging
from pathlib import Path
from typing import Self

from bs4 import BeautifulSoup, Tag
from markdownify import markdownify

from quarry.ingestion.section_splitter import SectionSplitter
from quarry.ingestion.text_splitter import (
    read_text_with_fallback,
    sections_to_pages,
)
from quarry.models import PageContent, PageType

logger = logging.getLogger(__name__)

SUPPORTED_HTML_EXTENSIONS = frozenset({".html", ".htm"})

# Tags whose content is boilerplate, not article knowledge.
_BOILERPLATE_TAGS = frozenset(
    {"script", "style", "nav", "header", "footer", "aside", "form", "noscript"}
)


class HtmlExtractor:
    """Extract pages from HTML files.

    Implements ``FormatExtractor`` protocol.  Strips boilerplate tags,
    converts to Markdown, then splits on headings or blank lines.

    Also exposes ``extract_from_html`` for the URL ingestion path,
    which processes raw HTML strings rather than files.
    """

    def __new__(cls) -> Self:
        return super().__new__(cls)

    def extract_pages(
        self,
        path: Path,
        *,
        document_name: str | None = None,
    ) -> list[PageContent]:
        """Parse an HTML file into Markdown sections for embedding."""
        suffix = path.suffix.lower()
        if suffix not in SUPPORTED_HTML_EXTENSIONS:
            msg = f"Unsupported HTML format: {suffix}"
            raise ValueError(msg)

        resolved_name = document_name or path.name
        html_text = read_text_with_fallback(path)
        return self.extract_from_html(html_text, resolved_name, str(path.resolve()))

    def extract_from_html(
        self,
        html: str,
        document_name: str,
        document_path: str,
    ) -> list[PageContent]:
        """Parse raw HTML into Markdown sections for embedding.

        Strategy:
          1. Parse with BeautifulSoup.
          2. Extract ``<title>`` text.
          3. Remove boilerplate tags (script, style, nav, etc.).
          4. Convert cleaned body to Markdown via markdownify.
          5. Prepend title as ``# Heading`` only if the body has no headings.
          6. Split on Markdown headings, or fall back to blank-line paragraphs.

        Args:
            html: Raw HTML content.
            document_name: Identifier stored with each chunk.
            document_path: Source location (file path or URL).

        Returns:
            List of PageContent objects, one per section.  Empty list when the
            HTML contains no extractable content.
        """
        logger.debug("Processing HTML: %s", document_name)

        soup = BeautifulSoup(html, "html.parser")

        title = self._extract_title(soup)
        self._strip_boilerplate(soup)

        body = soup.find("body")
        content_root = body if isinstance(body, Tag) else soup
        markdown = self._html_to_markdown(str(content_root))

        if title and not self._has_markdown_headings(markdown):
            markdown = f"# {title}\n\n{markdown}"

        headed = self._has_markdown_headings(markdown)
        splitter = SectionSplitter.markdown() if headed else SectionSplitter.plain()
        sections = splitter.split(markdown)

        if not sections:
            return []

        return sections_to_pages(
            sections, document_name, document_path, PageType.SECTION
        )

    @staticmethod
    def _strip_boilerplate(soup: BeautifulSoup) -> None:
        """Remove boilerplate tags from the DOM in place."""
        for tag in soup.find_all(_BOILERPLATE_TAGS):
            tag.decompose()

    @staticmethod
    def _extract_title(soup: BeautifulSoup) -> str:
        """Extract and return the <title> text, or empty string."""
        title_tag = soup.find("title")
        if isinstance(title_tag, Tag):
            return str(title_tag.get_text(strip=True))
        return ""

    @staticmethod
    def _html_to_markdown(html: str) -> str:
        """Convert an HTML fragment to Markdown via markdownify."""
        md: str = markdownify(html, heading_style="ATX", bullets="-")
        return md

    @staticmethod
    def _has_markdown_headings(text: str) -> bool:
        """Check whether text contains any ATX-style headings (# ...)."""
        for line in text.splitlines():
            stripped = line.lstrip()
            if stripped.startswith("#") and len(stripped) > 1 and stripped[1] in " #":
                return True
        return False
