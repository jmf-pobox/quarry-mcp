...
# Paths whose *last component* is one of these are always secret regardless
# of location.  Kept as lower-case strings for case-insensitive matching.
...
# Suffixes that mark a filename as a secret regardless of stem.  ``.pem``
# and ``.key`` cover PEM-formatted TLS keys, CSRs, and generic key files.
_SECRET_SUFFIXES = frozenset({".pem", ".key"})

# Path fragments that mark the containing directory as secret.  ``.ssh``
# covers every user's SSH tree (``id_*``, ``known_hosts``, ``config``);
# ``.aws`` covers ``.aws/credentials`` and ``.aws/config``.
_SECRET_DIRS = frozenset({".ssh", ".aws"})
...
class ReadCaptureFilter:
    ...
    def _is_in_tree(self, path: Path, cwd: str) -> bool:
        """Return whether *path* is under a registered collection covering *cwd*."""
    ...
    @classmethod
    def _is_secret_path(cls, path: Path) -> bool:
        """Return whether *path*'s name or ancestry matches the secret denylist."""
        parts_lower = tuple(p.lower() for p in path.parts)
        if any(part in _SECRET_DIRS for part in parts_lower):
            return True
        name_lower = path.name.lower()
        if name_lower in _SECRET_BASENAMES:
            return True
