...
```bash
curl -fsSL https://raw.githubusercontent.com/punt-labs/quarry/9d500df/install.sh | sh
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/punt-labs/quarry/9d500df/install.sh | sh -s -- --no-plugin
```

Where a flag cannot be passed (CI templating a bare `curl … | sh`), set `QUARRY_NO_PLUGIN=1` — honored only when exactly `1`:

```bash
curl -fsSL https://raw.githubusercontent.com/punt-labs/quarry/9d500df/install.sh | QUARRY_NO_PLUGIN=1 sh
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/punt-labs/quarry/9d500df/install.sh -o install.sh
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/punt-labs/quarry/9d500df/install.sh | sh -s -- --network
```
