...
\begin{itemize}
  ...
  \item Make reading a matched document's full content --- not just the
    ...
    \texttt{quarry-menubar} already proved is the right shape (\S UC-2,
    \S UC-3).
...
\begin{enumerate}
  ...
  \item \textbf{Still open; carried forward as Chapter 2, \S2.5 Decision
    ...
    reachability --- is FD headroom, FTS index health, and sync recency
    (all already in \texttt{quarry doctor}'s output) the right read-only
    subset, or is that too much detail for a glance-and-close use case?
...
\texttt{routes/show.py} (\texttt{route\_table.py} only wires route specs
to these handler methods; the handler bodies, and the service calls
worth reusing, live in \texttt{routes/}) --- in-process, never looping
back through its own HTTP API over loopback TLS to reach code already
running in the same process. This is a decision, not an oversight worth
...
\begin{enumerate}
  ...
  \item \textbf{Status-view scope beyond reachability, carried forward
    ...
    reachability --- FD headroom, FTS index health, and sync recency are
    all already in \texttt{quarry doctor}'s output and are candidates,
    but including all three may be more detail than a glance-and-close
    status view warrants. Genuinely undecided; needs a ruling before
