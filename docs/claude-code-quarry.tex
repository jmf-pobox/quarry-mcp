...
\begin{enumerate}
  \item \textbf{Knowledge before work}: the codebase is
    indexed before Claude begins processing.  The
    SessionStart hook auto-registers and syncs the working
    directory.
  ...
  \item \textbf{Capture toggles are respected}: each auto-capture
    ...
    disabled via \texttt{.punt-labs/quarry/config.md}.
\end{enumerate}
...
\begin{zed}
  [TOOLNAME, TOOLINPUT, TOOLOUTPUT, PROMPT, \\
  \quad RESPONSE, AGENTID, SESSIONID, CONTEXTCHUNK]
\end{zed}
...
\begin{schema}{State}
  ...
  %
  % --- Capture config (from .punt-labs/quarry/config.md) ---
  %
...
\begin{enumerate}
  ...
  \item \textbf{Compaction flag lifecycle}:
    CaptureBeforeCompact sets compactionSaved to ztrue.
    CompactContext resets it to zfalse.  This tracks whether
    the most recent compaction was preceded by a capture.
...
\textbf{Note on the compaction safety property:} The Z spec
does \emph{not} enforce that capture must precede compaction
--- it is possible to compact without capturing (the operations
are independent).  This reflects reality: Claude Code fires
the pre-compact hook before compaction, but the hook is
non-blocking.  The compactionSaved flag lets us \emph{observe}
whether the capture occurred, not \emph{enforce} it.
...
{\small
\begin{tabular}{llp{3.2cm}p{2.8cm}l}
\hline
\textbf{Hook} & \textbf{Matcher} & \textbf{Operation} &
  \textbf{Effect} & \textbf{Blk} \\
\hline
SessionStart & * & StartSession\-WithSync/\-NoSync &
  Auto-register + sync & No \\
SessionStart & * & InjectKnowledge\-Hint &
  Project standards & No \\
PostToolUse & WebFetch & CaptureWebFetch &
  Ingest URL & No \\
PostToolUse & Edit|Write & InjectKnowledge\-Hint &
  File-type hints & No \\
PostToolUse & Bash & InjectKnowledge\-Hint &
  Workflow hints & No \\
PostToolUse & quarry\_\_use$^\dagger$ & SwitchDatabase &
  Change active DB & No \\
PostToolUse & quarry\_\_find$^\dagger$ & SearchKnowledge &
  Inject results & No \\
PostToolUse & quarry\_\_*$^\dagger$ & (suppress output) &
  Format display & No \\
PreCompact & * & CaptureBefore\-Compact &
  Save session notes & No \\
SessionEnd$^\ddagger$ & * & EndSession &
  Clear state & No \\
\hline
\end{tabular}
}
...
\noindent
$^\dagger$Conceptual matcher; actual regex is
\texttt{mcp\_\_(plugin\_quarry...)?quarry(-proxy)?\_\_.*}
(see \texttt{hooks.json}).\\
$^\ddagger$Not yet wired --- no SessionEnd hook is registered.

% ============================================================
\section{Implementation Validation}
% ============================================================

This section cross-references the Z specification against
the current quarry implementation: the hook config, shell
scripts in the hooks directory, and the Python handlers.

\subsection{Hook Wiring Status}

All three knowledge capture handlers are implemented in
\texttt{src/quarry/hooks.py} and wired via shell scripts in
\texttt{plugin/hooks/hooks.json}.

{\small
\begin{tabular}{lp{3cm}p{5cm}l}
\hline
\textbf{Component} & \textbf{Hook Event} & \textbf{Files} &
  \textbf{Status} \\
\hline
Auto-register + sync & SessionStart &
  session-start.sh, session-sync.sh, hooks.py & Correct \\
Output formatting & PostToolUse (quarry) &
  suppress-output.sh & Correct \\
Web fetch capture & PostToolUse (WebFetch) &
  web-fetch.sh, hooks.py & Correct \\
Compaction capture & PreCompact &
  pre-compact.sh, hooks.py & Correct \\
\hline
\end{tabular}
}
...
\textbf{Knowledge hint injection} is the most significant unbuilt
capability.  It requires a hint resolution engine that maps the
current activity (tool name, file path, command) to relevant quarry
collections, plus careful scoping: hints must be short (1--3
sentences) to avoid flooding context.

An earlier attempt at convention hints via a PreToolUse hook
accumulator (see ADR in \texttt{DESIGN.md}) was implemented and
later removed (PR \#113) in favor of a future semantic approach.
...
\hline
StartSessionWithSync & hooks.py + session-sync.sh & Correct \\
StartSessionNoSync & hooks.py (config check) & Correct \\
SubmitPrompt / RejectPrompt & (base) & Correct \\
Tool call operations & (base) & Correct \\
CaptureWebFetch & hooks.py + web-fetch.sh & Correct \\
SwitchDatabase & MCP tool & Correct \\
SearchKnowledge & MCP tool & Correct \\
InjectKnowledgeHint & --- & \textbf{Not built} \\
CaptureBeforeCompact & hooks.py + pre-compact.sh & Correct \\
