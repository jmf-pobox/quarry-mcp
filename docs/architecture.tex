...
--- even on localhost.  (\texttt{mcp-proxy} remains a supported,
separately-installed tool for other consumers of the REST API; it is
simply no longer on quarry's own MCP path.)
...
Storage       & \modpath{db/storage.py}        & LanceDB connection,
                database discovery, and size formatting. \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Module} & \textbf{File} & \textbf{Responsibility} \\
\midrule
Pipeline    & \modpath{ingestion/pipeline.py}    & Orchestrator:
              dispatch by format, chunk, embed, store.  Entry points
...
URL ingester & \modpath{ingestion/url\_ingester.py} & Fetch, parse HTML,
              chunk, embed, store.  \texttt{ingest\_url} takes an opt-in
              \texttt{content\_scrubber} (see PII redaction). \\
...
Session transcripts and WebFetch auto-captures are scrubbed at write
time before any bytes reach a git-tracked (and soon pushed) surface
(DES-036).
...
Capture writer & \modpath{capture.py}       & \texttt{CaptureWriter} ---
                 the single choke point both \texttt{.md} producers
                 route through.  Scrubs, then atomically writes. \\
...
MCP Server  & \modpath{mcp\_server.py}  & \texttt{quarry mcp}: a FastMCP
              \emph{stdio} server whose eleven tool bodies are thin
              \texttt{QuarryClient} calls over the daemon's \texttt{/v1}
              API.  Imports \emph{no} engine --- loads zero
...
Hooks       & \modpath{hooks.py}        & Claude Code plugin hooks:
              ...
              (auto-ingest), pre-compact (capture transcript).  Pure
              clients (\texttt{QuarryClient}); all fail-open. \\
...
Enable     & \modpath{enable.py}     & Project activation and
             deactivation: register directory, bootstrap ethos
             memory extensions, write per-project config. \\
...
\emph{stdio} server (\modpath{mcp\_server.py}) whose eleven tool bodies
...
supported tool for other REST-API consumers).
...
\texttt{/v1/ingest}           & POST   & Ingest file or URL (returns 202) & Yes & async \\
...
\begin{enumerate}
  ...
  \item \textbf{BM25 full-text search} --- LanceDB's \emph{native}
    inverted index on the \texttt{text} column
    (\texttt{create\_fts\_index(..., use\_tantivy=False)}, the default),
    created during schema migration.  There is no separate Tantivy
    \texttt{IndexWriter} process or thread pool; the index rides the
    daemon's capped LanceDB runtime (DES-045c).  (Earlier ADRs --- DES-017,
    DES-023 --- name the backend ``Tantivy''; that name is legacy, the
    shipped engine is LanceDB-native.)
\end{enumerate}
...
1.3 minimum) when it is used by other REST-API consumers; it is no longer
...
\textbf{Single write choke point (\modpath{capture.py}).}  Both
\texttt{.md} producers --- the PreCompact hook (\modpath{hooks.py}) and
the backfill path (\modpath{backfill.py}) --- route through one
\texttt{CaptureWriter}.  Scrubbing runs to completion before an atomic
...
text via an opt-in \texttt{content\_scrubber} parameter on
\texttt{ingest\_url} (\modpath{ingestion/pipeline.py}).  The parameter
defaults to \texttt{None}: user-initiated \texttt{quarry ingest <url>},
...
server via TOFU certificate pinning and writes the mcp-proxy config.
...
PostToolUse   & WebFetch & Auto-ingest fetched URL (scrubbed) &
  No \\
PreCompact    & * & Capture transcript as session notes (scrubbed) &
  No \\
\bottomrule
\end{tabular}
\end{center}
...
(avoids re-fetch, reduces SSRF surface).  Both ingress branches (primary
and the \texttt{ingest\_url} re-fetch fallback) scrub page text and
redact the URL metadata before content reaches the pushable
\texttt{web-captures} collection (DES-036).
...
500{,}000 characters.  The capture file is written through the shared
\texttt{CaptureWriter}, which scrubs secrets, PII, and profanity before
the atomic write (DES-036).
...
\texttt{ingest}, \texttt{remember}, \texttt{delete}, \texttt{sync},
every \texttt{list} subcommand, and directory
\texttt{register}/\texttt{deregister} --- route remotely.  \texttt{use}
...
Tier 3: Surface wiring & Fully mocked / hermetic in-process ASGI
  ...
  \texttt{test\_http\_server}, \texttt{test\_client\_config},
...
ingest (URL)        & fire-and-forget & sync & fire-and-forget
                    & fire-and-forget \\
ingest (file)       & ---    & sync & ---  & fire-and-forget \\
sync                & fire-and-forget & sync & fire-and-forget
                    & fire-and-forget \\
use (switch db)     & sync   & sync & sync & sync \\
optimize            & fire-and-forget & sync & fire-and-forget & --- \\
backfill-sessions   & fire-and-forget & sync & fire-and-forget & --- \\
\end{longtable}
...
\textbf{Mutating operations} are all fire-and-forget on HTTP.  Every
mutating endpoint returns 202~Accepted with a \field{task\_id}; work
runs in a background \texttt{asyncio.Task}.  This eliminates the
15-second CLI timeout as a failure mode for any write operation.
...
\begin{enumerate}
  \item \texttt{POST} validates input, creates a \texttt{TaskState}
    record on the server context, spawns an \texttt{asyncio.Task}
    wrapping \texttt{run\_in\_threadpool()}, and returns HTTP~202
    with \field{task\_id} and \field{status}~=~\texttt{accepted}.
